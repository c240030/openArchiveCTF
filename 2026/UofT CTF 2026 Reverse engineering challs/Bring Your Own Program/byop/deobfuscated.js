...
function c(b) {
  const N = q.statSync(b)
  if (!N.isFile()) {
    return ''
  }
  const FN = q.openSync(b, q.constants.O_RDONLY)
  try {
    const Fi = Math.min(N.size, Z),
      Fq = Buffer.alloc(Fi),
      Ft = q.readSync(FN, Fq, 0, Fi, 0)
...
FA.rs = MemberExpression
FA.k = Identifier
FA.v = Identifier
FA.d = MemberExpression
FA.si = MemberExpression
FB.c = ArrayExpression
FB.m = Literal
Running Desequence transformer
Running MemberExpressionCleaner transformer
...
Running Simplify transformer
Running DeadCode transformer
Deobfuscation complete in 0m 0s 296ms
*/