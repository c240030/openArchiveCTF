...
@section('content')
  <section class="ow-card" style="max-width: 520px; margin: 0 auto;">
    ...
    <form method="POST" action="{{ url('/register') }}" class="grid" style="gap: 10px;">
      @csrf

      <div class="ow-field">
        <label class="ow-label" for="name">Name</label>
        <input id="name" name="name" class="ow-input" value="{{ old('name') }}" required minlength="2" maxlength="80"
               placeholder="Kenshin S." autocomplete="name">
      </div>

      <div class="ow-field">
        <label class="ow-label" for="email">Email</label>
        <input id="email" type="email" name="email" class="ow-input" value="{{ old('email') }}" required
      ...
      <div style="display:flex; align-items:center; justify-content:space-between; gap: 12px; margin-top: 6px;">
        ...
        <div class="ow-subtle">Have an account? <a href="{{ route('login') }}" class="ow-link">Login</a></div>
      </div>
    </form>
  </section>
@endsection
