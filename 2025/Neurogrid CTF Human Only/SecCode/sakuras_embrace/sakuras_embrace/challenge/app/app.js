import express from "express";
import path from "path";
import { fileURLToPath } from "url";
import cookieParser from "cookie-parser";
import morgan from "morgan";
import { allItems, getItemsByIds } from "./db.js";
import dotenv from 'dotenv';
dotenv.config();


const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

const app = express();
app.use("/public", express.static(path.join(__dirname, "public")));
app.use("/images", express.static(path.join(__dirname, "public/images")));


const PORT = process.env.PORT || 5000;
const COOKIE_SECRET = process.env.SESSION_SECRET || "senmonten-secret";

const yenFmt = new Intl.NumberFormat("ja-JP", { style: "currency", currency: "JPY" });
app.locals.yen = (n) => yenFmt.format(n);

// Helpers for strict input validation
function parseItemId(value) {
  const num = Number(value);
  return Number.isInteger(num) && num > 0 ? num : null;
}

function parseQuantity(value, { allowZero = false } = {}) {
  if (value === undefined || value === null) return null;
  const str = String(value).trim();
  if (!/^\d+$/.test(str)) return null;
  const num = Number(str);
  if (!Number.isSafeInteger(num)) return null;
  if (num > 1_000_000) return null;
  if (allowZero ? num < 0 : num <= 0) return null;
  return num;
}

// Cart management functions
function readCart(req) {
  const c = req.signedCookies?.cart;
  if (!c) return {};
  try {
    const obj = JSON.parse(c);
    if (typeof obj !== "object" || !obj) return {};

    // Sanitize cart data - ensure all keys/values are valid integers
    const sanitized = {};
    for (const [key, value] of Object.entries(obj)) {
      const itemId = parseItemId(key);
      const quantity = parseQuantity(value);

      if (itemId !== null && quantity !== null) {
        sanitized[itemId] = quantity;
      }
    }
    return sanitized;
  } catch {
    return {};
  }
}

function writeCart(res, cart) {
  res.cookie("cart", JSON.stringify(cart), {
    httpOnly: true,
    signed: true,
    sameSite: "lax",
    secure: process.env.NODE_ENV === "production",
    maxAge: 7 * 24 * 60 * 60 * 1000
  });
}

// Calculate cart totals using integer quantities
function buildCartComputation(cart) {
  const ids = Object.keys(cart).map(Number).filter(Number.isFinite);
  const items = getItemsByIds(ids).sort((a, b) => a.id - b.id);

  const lines = items.map((it) => {
    const rawQty = cart[it.id];
    const qtyNum = typeof rawQty === "number" ? rawQty : parseQuantity(rawQty);

    if (qtyNum === null) {
      return { ...it, qtyExpr: "0", qtyNum: 0, lineExpr: `${it.price_yen}*0`, lineTotal: 0 };
    }

    const lineTotal = it.price_yen * qtyNum;
    return { ...it, qtyExpr: String(qtyNum), qtyNum, lineExpr: `${it.price_yen}*${qtyNum}`, lineTotal };
  });

  const total = lines.reduce((sum, l) => sum + (Number.isFinite(l.lineTotal) ? l.lineTotal : 0), 0);
  const formula = lines.map((l) => l.lineExpr).join(" + ") || "0";

  return { lines, total, formula };
}

// Middleware setup
app.set("views", path.join(__dirname, "views"));
app.set("view engine", "pug");
// Disable Pug code execution features for security
app.locals.compileDebug = false;
app.locals.cache = true;
app.use(morgan("tiny"));
app.use(express.urlencoded({ extended: true }));
app.use(cookieParser(COOKIE_SECRET));


// Set up global template variables
app.use((req, res, next) => {
  res.locals.palette = {
    kinari: "#f5f1e8",
    sumi: "#2b2b2b",
    nezumi: "#efeee9",
    ai: "#0f4c81",
    aka: "#d23b2f"
  };
  const cart = readCart(req);
  const computed = buildCartComputation(cart);
  res.locals.cart = cart;
  res.locals.cartFull = computed;
  next();
});

app.use('/build', express.static(path.join(__dirname, 'public', 'build'), {
  immutable: true,
  maxAge: '1y',
}));

// Routes


// Display all items on homepage
app.get("/", (req, res) => {
  const items = allItems();
  res.render("index", { title: "Senmonten", items });
});


// Add item to cart with quantity expression
app.post("/cart/add", (req, res) => {
  const id = parseItemId(req.body.itemId);
  const quantity = parseQuantity(req.body.quantity);

  if (id === null || quantity === null) {
    return res.redirect("/");
  }

  const cart = readCart(req);
  const currentQty = typeof cart[id] === "number" ? cart[id] : 0;
  cart[id] = currentQty + quantity;
  writeCart(res, cart);
  return res.redirect(`/challenge/#added-${id}`);

});


// Update item quantity in cart
app.post("/cart/update", (req, res) => {
  const id = parseItemId(req.body.itemId);
  const quantity = parseQuantity(req.body.quantity, { allowZero: true });

  if (id === null || quantity === null) {
    return res.redirect("/");
  }

  const cart = readCart(req);
  if (quantity === 0) {
    delete cart[id];
  } else {
    cart[id] = quantity;
  }

  writeCart(res, cart);
  return res.redirect(`/challenge/#cart-item-${id}`);
});


// Remove item from cart
app.post("/cart/remove", (req, res) => {
  const id = parseItemId(req.body.itemId);
  if (id === null) return res.redirect("/");
  const cart = readCart(req);
  delete cart[id];
  writeCart(res, cart);
  return res.redirect(`/challenge/#removed-${id}`);
});

// API: Get all available items
app.get("/api/v1/items", (req, res) => res.json({ items: allItems() }));

// API: Get current cart with calculations
app.get("/api/v1/cart", (req, res) => {
  const cart = readCart(req);
  const full = buildCartComputation(cart);

  res.json({
    formula: full.formula,
    items: full.lines.map((l) => ({
      id: l.id, name: l.name, price_yen: l.price_yen, 
      qtyExpr: l.qtyExpr, lineExpr: l.lineExpr, lineTotal: l.lineTotal
    })),
    total: full.total
  });
});


// Process checkout and clear cart
app.post("/cart/checkout", (req, res) => {
  writeCart(res, {});
  return res.redirect("/challenge/#purchased");
});

// Start server
app.listen(PORT, () => {
  console.log(`Senmonten listening on http://localhost:${PORT}`);
});
